...
export default async function GuildWishlistPage({
  params,
}: {
  ...
  // Get all guild character IDs first, then filter wishlist items
  const guildCharacterIds = await prisma.character.findMany({
    where: { guildId: guild.id },
    select: { id: true },
  });
  const charIds = guildCharacterIds.map((c) => c.id);

  const items = await prisma.wishlistItem.findMany({
    where: { obtained: false, characterId: { in: charIds } },
    include: {
      character: {
                      ...
                      <div className="flex flex-wrap gap-3">
                        {itemEntries.map((entry) => {
                          ...
                          return (
                            <Link
                              key={entry.id}
                              href={`${base}/wishlist/${entry.character.id}`}
