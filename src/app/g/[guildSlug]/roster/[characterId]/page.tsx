...
export default async function GuildCharacterDetailPage({
  params,
}: {
  params: Promise<{ guildSlug: string; characterId: string }>;
}) {
  const { guildSlug, characterId } = await params;
  ...
  if (!character || character.guildId !== guild.id) notFound();

  const canEditWishlist = isOwner(session, character.userId) || isRaidLeader(membership);
        ...
        <TabsContent value="wishlist" className="mt-4">
          ...
          {character.wishlistItems.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              <Link
                href={`${base}/wishlist/${characterId}`}
              ...
              {character.performances.map((perf) => (
                <Card key={perf.id}>
                  <CardContent className="flex items-center justify-between py-3">
                    <div>
                      <Link
                        href={`${base}/logs/${perf.raidLog.reportCode}`}
              ...
              {character.attendances.map((att) => {
                ...
                return (
                  <Card key={att.id}>
                    <CardContent className="flex items-center justify-between py-3">
                      <Link
                        href={`${base}/calendar/${att.raidEvent.id}`}
